...
Section Gen_Sum_Gen_Prod.
  ...
  Program Instance Gen_Sum_of_Gen_Prod : General_Sum (C ^op) A objs p :=
    {
      GS_Inj := @GP_Proj _ _ _ _ _ _ GP;

      GS_Sum_morph_ex := λ a H, @GP_Prod_morph_ex _ _ _ _ _ _ GP a H;
      
      GS_Sum_morph_com := λ a H b, @GP_Prod_morph_com _ _ _ _ _ _ GP a H b;

      GS_Sum_morph_unique := λ a H f g, @GP_Prod_morph_unique _ _ _ _ _ _ GP a H f g
    }.

End Gen_Sum_Gen_Prod.

Section Has_Gen_Sum_op_Has_Gen_Prod.
  Context `(C : Category Obj Hom) (A : Type) (HGP : Has_General_Products C A).

  Program Instance Has_Gen_Prod_op_Has_Gen_Sum : Has_General_Sums (C ^op) A :=
    {
      Gen_Sum_of := Gen_Prod_of;
      Gen_Sum_sum := λ a, Gen_Sum_of_Gen_Prod _ _ _ _ (Gen_Prod_prod a)
    }.

End Has_Gen_Sum_op_Has_Gen_Prod.

Section Has_Restr_Gen_Sum_op_Has_Restr_Gen_Prod.
  Context `(C : Category Obj Hom) (P : Type → Prop) (HGP : Has_Restricted_General_Products C P).
...
Section Has_All_Gen_Sum_op_Has_All_Gen_Prod.
  Context `(C : Category Obj Hom) (HGP : Has_All_General_Products C).
...
Section Prod_Sum.
  Context `(C : Category Obj Hom) (A : Type) (objs : A → Obj) (s : Obj) (GS : General_Sum C A objs s).

  Program Instance Gen_Prod_of_Gen_Sum : General_Product (C ^op) A objs s :=
    {
      GP_Proj := @GS_Inj _ _ _ _ _ _ GS;

      GP_Prod_morph_ex := λ a H, @GS_Sum_morph_ex _ _ _ _ _ _ GS a H;
      
      GP_Prod_morph_com := λ a H b, @GS_Sum_morph_com _ _ _ _ _ _ GS a H b;

      GP_Prod_morph_unique := λ a H f g, @GS_Sum_morph_unique _ _ _ _ _ _ GS a H f g
    }.
...
Section Has_Gen_Prod_op_Has_Gen_Sum.
  Context `(C : Category Obj Hom) (A : Type) (HGP : Has_General_Sums C A).

  Program Instance Has_Gen_Sum_op_Has_Gen_Prod : Has_General_Products (C ^op) A :=
    {
      Gen_Prod_of := Gen_Sum_of;
      Gen_Prod_prod := λ a, Gen_Prod_of_Gen_Sum _ _ _ _ (Gen_Sum_sum a)
    }.

End Has_Gen_Prod_op_Has_Gen_Sum.

Section Has_Restr_Gen_Prod_op_Has_Restr_Gen_Sum.
  Context `(C : Category Obj Hom) (P : Type → Prop) (HGP : Has_Restricted_General_Sums C P).
...
Section Has_All_Gen_Prod_op_Has_All_Gen_Sum.
  Context `(C : Category Obj Hom) (HGP : Has_All_General_Sums C).
