...
Section Prod_Functor_NatTrans_id.
  ...
  Proof. apply NatTrans_eq_simplify; trivial. Qed.
...
Section Prod_Functor_NatIso.
  ...
  Next Obligation.
    ...
    change ((N ⁻¹)%morphism ∘ (iso_morphism N))%nattrans
      with (N⁻¹ ∘ N)%morphism.
    change ((N' ⁻¹)%morphism ∘ (iso_morphism N'))%nattrans
      with (N'⁻¹ ∘ N')%morphism.
  ...
  Next Obligation.
    cbn.
    rewrite Prod_Functor_NatTrans_compose.
    change ((iso_morphism N) ∘ (N ⁻¹)%morphism)%nattrans with
        (N ∘ N⁻¹)%morphism.
    change ((iso_morphism N') ∘ (N' ⁻¹)%morphism)%nattrans with
        (N' ∘ N'⁻¹)%morphism.
...
Section Fix_Bi_Func_1_NatIso.
  ...
  Proof.
    ...
    cbn.
    change (Trans (inverse_morphism N) (G, c) ∘
            Trans (iso_morphism N) (G, c))%morphism with
        (Trans ((inverse_morphism N) ∘ (iso_morphism N)) (G, c)).
    cbn_rewrite (left_inverse N); trivial.
  Qed.
  Next Obligation.
  Proof.
    apply NatTrans_eq_simplify; extensionality c; cbn.
    change (Trans (iso_morphism N) (G, c) ∘
            Trans (inverse_morphism N) (G, c))%morphism with
        (Trans ((iso_morphism N) ∘ (inverse_morphism N)) (G, c)).
...
Section Fix_Bi_Func_2_NatTrans.
  ...
  Next Obligation.
  Proof. intros; apply (@Trans_com _ _ _ _ N). Qed.
  Next Obligation.
  Proof. intros; apply (@Trans_com_sym _ _ _ _ N). Qed.
...
Section Fix_Bi_Func_1_Functor_id_swap_NatIso.
  ...
  Program Definition Fix_Bi_Func_1_Functor_id_swap_NatIso :
    ((@Fix_Bi_Func_1
        (Func_Cat _ _) _ _ G ( F' ∘ (Prod_Functor (Functor_id _) F)))%functor ≃
...
Section Fix_Bi_Func_2_Functor_id_swap_NatIso.
  ...
  Program Definition Fix_Bi_Func_2_Functor_id_swap_NatIso :
    (((@Fix_Bi_Func_2
         _ (Func_Cat _ _) _ G (F' ∘ (Prod_Functor F (Functor_id _))))%functor) ≃
...
Section Fix_Bi_1_Func_Prod_Func_NatIso.
  ...
  Program Definition Fix_Bi_1_Func_Prod_Func_NatIso :
    ...
    ((Fix_Bi_Func_1 (F _o x) (G ∘ (Prod_Functor (Functor_id C) F'))))%functor
    )%natiso :=
    {|
...
Section Fix_Bi_2_Func_Prod_Func_NatIso.
  ...
  Program Definition Fix_Bi_2_Func_Prod_Func_NatIso :
    (((Fix_Bi_Func_2 x (G ∘ (Prod_Functor F F')))%functor) ≃
      (Fix_Bi_Func_2 (F' _o x) (G ∘  (Prod_Functor F (Functor_id D))))%functor)
      %natiso :=
    {|
      iso_morphism := {|Trans := fun c => id|};
      inverse_morphism := {|Trans := fun c => id|}
