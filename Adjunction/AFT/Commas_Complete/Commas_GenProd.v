...
From Categories Require Import Functor.Functor Const_Func Functor.Functor_Ops.
From Categories Require Import NatTrans.Main.
From Categories Require Import Ext_Cons.Comma.
From Categories.Basic_Cons Require Import Terminal Equalizer Limits.
...
Section Commas_GenProd.
  ...
  Section fun_conv.
    Context {A : Type} (f : A → (Comma (Const_Func 1 x) G)).
  ...
  Section Comma_GenProd_Cone_Morph_ex.
          ...
          Trans :=
            fun c =>
              {|
                CMH_left := tt;
                CMH_right :=
                  match c as u return (CMO_trg (Cn _o u)%object --> _)%morphism
                  with
                    tt =>
                    Trans
                      (LRKE_morph_ex
                         (GenProd_fun_conv) (Cone_f_Cone_fun_conv_f Cn))
                      tt
                  end
              |}
        |}
    ...
    Proof.
      ...
      cbn_rewrite
        (f_equal
           (fun w : ((Cone_f_Cone_fun_conv_f Cn)
                     ∘ Functor_To_1_Cat (Discr_Cat A)
                     --> Discr_Func (fun_conv f))%nattrans
            => Trans w z)
           (cone_morph_com
        ...
        f_equal
          (fun w :
                 ((Cn ∘ Functor_To_1_Cat (Discr_Cat A))
                    -->
                    @Discr_Func (Comma (Const_Func 1 x) G) _ f)%nattrans
           => CMH_right (Trans w z)
          )
          (cone_morph_com h)
      ).
  ...
  Proof.
    ...
    (Trans (Cone_Morph_to_Comma_GenProd_Cone_TO_Cone_Morph_to_GenProd_fun_conv
              Cn h) tt).
    change (CMH_right (Trans h' tt)) with
    (Trans (Cone_Morph_to_Comma_GenProd_Cone_TO_Cone_Morph_to_GenProd_fun_conv
              Cn h') tt).
