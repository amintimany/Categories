...
Section Hom_Func_Twist.
   ...
   *)
  Theorem Hom_Func_Twist :
    (Hom_Func (C^op)) = (Hom_Func C) ∘ (Twist_Func C (C^op)).
  Proof.
...
Section Prod_Func_Hom_Func_NT.
  ...
  (** Given a natural transformation from ((Hom_Func C) ∘ (Prod_Functor F G)) to
      ((Hom_Func D) ∘ (Prod_Functor F' G')), we construct a natural
      transformation from ((Hom_Func C^op) ∘ (Prod_Functor G F)) to
...
Section Prod_Func_Hom_Func.
  Context {A B C D : Category}
          ...
          {G : B --> C}
          {G' : B --> D}
          (N : (((Hom_Func C) ∘ (Prod_Functor F G))%functor ≃
                ((Hom_Func D) ∘ (Prod_Functor F' G'))%functor)%natiso).
...
Section Hom_Func_to_Iso_Hom_Func.
  ...
  Program Definition Hom_Func_to_Iso_Hom_Func :
    ((Hom_Func C) -->
     ((Hom_Func D) ∘
       (Prod_Functor ((iso_morphism I)^op) (iso_morphism I))))%nattrans :=
  ...
  Program Definition Hom_Func_Cat_Iso :
    ((Hom_Func C) ≃
     ((Hom_Func D) ∘
       (Prod_Functor ((iso_morphism I)^op) (iso_morphism I)))%functor)%natiso :=
    {|
