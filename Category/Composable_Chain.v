...
(** A composable chain in a category from a to b is a single arrow from a to b or an arrow from a to some c provided that there is a composable chain from c to b.
...
(**
It doesn't matter if we first chain-compose two composable chains and then get the compostion of the resutlting chain or if we first compose individual chains and then composte the two resulting arrows.
*)
Theorem Compose_of_Chain_Compose (C : Category) (a b c : C) (ch1 : Composable_Chain C a b) (ch2 : Composable_Chain C b c) : ((Compose_of ch2) ∘ (Compose_of ch1))%morphism = Compose_of (Chain_Compose ch1 ch2).
Proof.
  induction ch1; auto.
  simpl.
  rewrite <- assoc.
