...
Section SubCategory.
  ...
  Program Definition SubCategory (Hom_Cri_id : ∀ a, Obj_Cri a → Hom_Cri _ _ (@id _ _ _ a)) (Hom_Cri_compose : ∀ {a b c} (f : Hom a b) (g : Hom b c), Hom_Cri _ _ f → Hom_Cri _ _ g → Hom_Cri _ _ (g ∘ f)) : Category (sig Obj_Cri) (λ a b, sig (Hom_Cri (proj1_sig a) (proj1_sig b))) :=
  {|
  ...
  Class SubCategory : Set :=
    {
      ...
      sub_cat := SC Hom_Cri_id Hom_Cri_compose
    }.

  Coercion sub_cat : SubCategory >-> Category. *)

End SubCategory.

Notation Wide_SubCategory C Hom_Cri := (SubCategory C (λ _, True) Hom_Cri).

Notation Full_SubCategory C Obj_Cri := (SubCategory C Obj_Cri (λ _ _ _, True)).
