...
We show that le implies Tle (and vise versa) and use this to convert
le to Tle and then eliminate Tle.
...
Type and not in Pop and hence we can't use proof-irrelevance.
...
Lemma le_Tle_S (n m : nat) (H : le n m) : le_Tle (le_S _ _ H) = Tle_S _ _ (le_Tle H).
Proof.
  apply Tle_is_HProp.
Qed.
...
Program Definition OmegaCat_Op_Func {C : Category}
  ...
  {|
    FO := OOF_O;
    FA := fun m n h => FA_fx OOF_O OOF_A _ _ (le_Tle h)
  |}
.
...
Next Obligation.
Proof.
  rewrite le_Tle_trans.
  generalize (le_Tle f) as t.
  generalize (le_Tle g) as t'.
  clear.
  induction t.
  + cbn; auto.
  + replace (Tle_trans c b (S m) t' (Tle_S b m t)) with (Tle_S _ _ (Tle_trans _ _ _ t' t)).
    cbn.
    rewrite IHt.
    auto.
    apply Tle_is_HProp.
...
(** Any functor from ωᵒᵖ to C is freely generated by its object map and
the image of the arrow map under (le_S _ _ (le_n n)) (proof of (le n (S n))). *)
Lemma OmegaCat_Op_Func_unique {C : Category} (F : ((ω^op) –≻ C)%functor) :
  F = OmegaCat_Op_Func (FO F) (fun n => FA F (le_S _ _ (le_n n))).
Proof.
  Func_eq_simpl.
  extensionality y.
  extensionality x.
...
(** Any functor from ω to C is freely generated by its object map and
the image of the arrow map under (le_S _ _ (le_n n)) (proof of (le n (S n))). *)
Lemma OmegaCat_Func_unique {C : Category} (F : (ω –≻ C)%functor) :
  F = OmegaCat_Func (FO F) (fun n => FA F (le_S _ _ (le_n n))).
Proof.
