...
Lemma OmegaCat_Op_Func_unique {C : Category} (F : ((ω^op) --> C)%functor) :
  F = OmegaCat_Op_Func (FO F) (fun n => F _a (Tle_S _ _ (Tle_n n)))%morphism.
Proof.
  Func_eq_simpl.
  extensionality y.
  extensionality x.
...
Lemma OmegaCat_Func_unique {C : Category} (F : (ω --> C)%functor) :
  F = OmegaCat_Func (FO F) (fun n => F _a (Tle_S _ _ (Tle_n n)))%morphism.
Proof.
