...
Program Definition Limit_Iso {J C : Category} {D : Functor J C} (l l' : Limit D) :
  (l ≡≡ l' ::> C)%morphism :=
  {|
    iso_morphism :=
      Trans
...
Definition Has_Restr_Limits (C : Category) (P : Card_Restriction) :=
  ∀ {J : Category} (D : Functor J C), P J → P (Arrow J) → Limit D.
...
Definition Limit_of {C D : Category} {H : Complete D} (F : Functor C D) : Limit F :=
...
Definition CoCone {J C : Category} (D : Functor J C) :=
  LoKan_Cone (Functor_To_1_Cat J^op) (Opposite_Functor D).

Definition CoCone_Morph {J C : Category} (D : Functor J C) Cn Cn' :=
  @LoKan_Cone_Morph _ _ (Functor_To_1_Cat J^op) _ (Opposite_Functor D) Cn Cn'.
...
Definition Has_Restr_CoLimits (C : Category) (P : Card_Restriction) :=
  ∀ {J : Category} (D : Functor J C), P J → P (Arrow J) → CoLimit D.
...
Definition CoLimit_of {C D : Category} {H : CoComplete D} (F : Functor C D) : CoLimit F :=
